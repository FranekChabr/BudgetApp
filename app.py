...
def generate_pie_chart(category_totals):
    """Generuje wykres kołowy dla kategorii wydatków"""
    if not category_totals:
        return None

    # Sortuj kategorie według wartości (od największej do najmniejszej)
    ...
    ax.set_title('Wydatki według kategorii', fontsize=16, fontweight='bold', pad=20)
...
def generate_bar_chart(category_totals):
    """Generuje wykres słupkowy dla kategorii wydatków"""
    if not category_totals:
        return None
    ...
    for bar, amount in zip(bars, amounts):
        ...
        ax.text(bar.get_x() + bar.get_width() / 2., height + height * 0.01,
                f'{amount:.2f} PLN', ha='center', va='bottom', fontsize=10)

    ax.set_xlabel('Kategorie', fontsize=12, fontweight='bold')
    ax.set_ylabel('Kwota (PLN)', fontsize=12, fontweight='bold')
    ax.set_title('Wydatki według kategorii', fontsize=16, fontweight='bold', pad=20)

    # Obróć etykiety kategorii jeśli są długie
...
def view_expenses():
    ...
    # jesli formularz nie zostal przeslany, ale mamy parametry GET (np. po przekierowaniu)
        ...
        if category_param:
            query = query.filter(Expense.category == category_param)
            filter_form.category.data = category_param

        if date_from_param:
            try:
                date_from = datetime.strptime(date_from_param, '%Y-%m-%d')
                query = query.filter(Expense.date >= date_from)
                filter_form.date_from.data = date_from.date()
            except ValueError:
                pass
        ...
        if date_to_param:
            try:
                ...
                query = query.filter(Expense.date <= date_to)
                filter_form.date_to.data = date_to.date()
            except ValueError:
                pass
    ...
    pie_chart = generate_pie_chart(category_totals)
    bar_chart = generate_bar_chart(category_totals)
